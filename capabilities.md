...
⚠️ The feature is tested and verified, but **no server is configured by default** — it stays
...
|---|---|---|
| ~~See images / screenshots~~ | ✅ **Solved in v3.0** — `analyze_image(path, question)` tool, one-shot call to an installed multimodal model (sequential loading; configure with `/vision-model`) | — |
| ~~RAG over local documents~~ | ✅ **Solved in v3.0** — `search_semantic(query)` tool, bge-m3 embeddings + stdlib SQLite index (no added dependency: neither ChromaDB nor FAISS), incremental re-indexing | — |
...
> ✅ Solved since this file was first written: persistent memory between sessions
...
>
> ✅ **Solved in v3.0** (see `DESIGN.md`): vision (`analyze_image`), local RAG
...
> ⚠️ Several models named in the re-benchmark history (`lfm2:24b-a2b`,

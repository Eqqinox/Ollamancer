...
- 🛠️ **Small-model reliability engineering** — retries + fallback for four confirmed Ollama tool-call failure signatures, chunked writes to avoid mid-JSON truncation, a one-time **model failover**, and a documented benchmark campaign across ~15 models.
